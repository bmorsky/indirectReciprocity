...
function sternjudge!(du,u,p,t)
        g = u[1]*u[4] + u[2]*u[5] + u[3]*u[6]
        g2 = u[1]*u[4]^2 + u[2]*u[5]^2 + u[3]*u[6]^2
        b = u[1]*(1-u[4]) + u[2]*(1-u[5]) + u[3]*(1-u[6])
        b2 = u[1]*(1-u[4])^2 + u[2]*(1-u[5])^2 + u[3]*(1-u[6])^2
        ...
        Icb = ϵ*Ocb + (1 - ϵ)*Odb
        Idb = (1 - e)*Odb + e*Ocb
        gx₊ = (1 - u[4])*(Icg*g + Icb*(1-g))
        gx₋ = u[4]*(1 - Icg)*((1-Icg)*g + (1-Icb)*(1-g))
        gx2₊ = (u[4] - u[7])*(Icg*g + Icb*(1-g))
        gx2₋ = u[7]*(1 - Icg)*((1-Icg)*g + (1-Icb)*(1-g))
        gy₊ = (1 - u[5])*(Idg*g + Idb*(1-g))
        gy₋ = u[5]*(1 - Idg)*((1-Idg)*g + (1-Idb)*(1-g))
        gy2₊ = (u[5] - u[8])*(Idg*g + Idb*(1-g))
        gy2₋ = u[8]*(1 - Idg)*((1-Idg)*g + (1-Idb)*(1-g))
        gz₊ = (1 - u[6])*(Icg*g2 + (g-g2)*Idg + b2*Idb + (b-b2)*Icb)
        gz₋ = u[6]*((1-Icg)*g2 + (g-g2)*(1-Idg) + b2*(1-Idb) + (b-b2)*(1-Icb))
        gz2₊ = (u[6] - u[9])*(Icg*g2 + (g-g2)*Idg + b2*Idb + (b-b2)*Icb)
        gz2₋ = u[9]*((1-Icg)*g2 + (g-g2)*(1-Idg) + b2*(1-Idb) + (b-b2)*(1-Icb))
        du[4] = τ*(gx₊ - gx₋)
        du[5] = τ*(gy₊ - gy₋)
        du[6] = τ*(gz₊ - gz₋)
        du[7] = τ*(gx2₊ - gx2₋)
...
for m = 0:5:100
        ...
        for n = 0:5:100-m
                ...
                #                 for gz = 0:0.25:1
                                        ...
                                        prob = ODEProblem(sternjudge!,u₀,(0.0,50.0))
                                        sol = solve(prob)
...
tax.scatter([0 0 1], linewidths=10, marker="o")
tax.show()
gcf()
tax.savefig("sternjudge_ternary")
clf()

## Third type of error case
e₃ = e

function sternjudge_e₃!(du,u,p,t)
        g = u[1]*u[4] + u[2]*u[5] + u[3]*u[6]
...
for m = 0:1:500
        ...
        for n = 0:1:500-m
                ...
                #                 for gz = 0:0.25:1
                                        ...
                                        gz=0.5
                                        u₀ = [x;y;z;gx;gy;gz]
                                        prob = ODEProblem(sternjudge_e₃!,u₀,tspan)
                                        sol = solve(prob)
