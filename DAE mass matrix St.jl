...
function staying!(du,u,p,t)
    ...
    gy2₊ = (u[2] - u[5])*Qdg*g
    gy2₋ = u[5]*(1-Qdg)*g
    gz₊ = (1 - u[3])*(Qcg*g2 + Qdg*(g-g2))
    gz₋ = u[3]*((1-Qcg)*g2 + (1-Qdg)*(g-g2))
    gz2₊ = (u[3] - u[6])*(Qcg*g2 + Qdg*(g-g2))
...
function DAE!(du,u,p,t)
    ...
    gy2₊ = (u[4] - u[7])*Qdg*g
    gy2₋ = u[7]*(1-Qdg)*g
    gz₊ = (1 - u[5])*(Qcg*g2 + Qdg*(g-g2))
    gz₋ = u[5]*((1-Qcg)*g2 + (1-Qdg)*(g-g2))
    gz2₊ = (u[5] - u[8])*(Qcg*g2 + Qdg*(g-g2))
...
tax.show()
tax.savefig(string("St_lambda",λ,"_bias",bias,"_DAE.png"))
gcf()
