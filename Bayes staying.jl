...
function staying!(du,u,p,t)
        ...
        gy2₊ = (u[5] - u[8])*Idg*g
        gy2₋ = u[8]*(1 - Idg)*g
        gz₊ = (1 - u[6])*(Icg*g2 + Idg*(u[1]*(u[4] - u[7]) + u[2]*(u[5] - u[8]) + u[3]*(u[6] - u[9])))
        gz₋ = u[6]*((1 - Icg)*g2 + (1 - Idg)*(u[1]*(u[4] - u[7]) + u[2]*(u[5] - u[8]) + u[3]*(u[6] - u[9])))
        gz2₊ = (u[6] - u[9])*(Icg*g2 + Idg*(u[1]*(u[4] - u[7]) + u[2]*(u[5] - u[8]) + u[3]*(u[6] - u[9])))
        gz2₋ = u[9]*((1 - Icg)*g2 + (1 - Idg)*(u[1]*(u[4] - u[7]) + u[2]*(u[5] - u[8]) + u[3]*(u[6] - u[9])))
        du[4] = τ*(gx₊ - gx₋)
        du[5] = τ*(gy₊ - gy₋)
        du[6] = τ*(gz₊ - gz₋)
        du[7] = τ*(gx2₊ - gx2₋)
...
for m = 0:1:numsims
        ...
        for n = 0:1:numsims-m
                ...
                #                 for gz = 0:0.25:1
                                        ...
                                        prob = ODEProblem(staying!,u₀,tspan)
                                        sol = solve(prob)
...
for m = 0:5:100
        ...
        for n = 0:5:100-m
                ...
                #                 for gz = 0:0.25:1
                                        ...
                                        prob = ODEProblem(staying!,u₀,(0.0,1e9))
                                        sol = solve(prob)
...
tax.show()
gcf()
tax.savefig("staying_ternary")
clf()

## Third type of error case
e₃ = e

function staying_e₃!(du,u,p,t)
        g = u[1]*u[4] + u[2]*u[5] + u[3]*u[6]
...
for m = 0:1:500
        ...
        for n = 0:1:500-m
                ...
                #                 for gz = 0:0.25:1
                                        ...
                                        gz=0.5
                                        u₀ = [x;y;z;gx;gy;gz]
                                        prob = ODEProblem(staying_e₃!,u₀,tspan)
                                        sol = solve(prob)
